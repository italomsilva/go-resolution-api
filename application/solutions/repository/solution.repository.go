...
import (
	"database/sql"
	"go-resolution-api/application/solutions/model"
)
...
func (repository *SolutionRepository) fromDatabase(rows *sql.Rows) ([]model.Solution, error) {
	var solutionsList []model.Solution
	for rows.Next() {
		solutionObj := model.Solution{}
		err := rows.Scan(
			&solutionObj.ID,
			&solutionObj.Title,
...
func (repository *SolutionRepository) GetAllSolutionsByProblemId(problemId string) ([]model.Solution, error) {
...
func (repository *SolutionRepository) GetSolutionById(id string) (*model.Solution, error) {
...
func (repository *SolutionRepository) CreateSolution(data *model.Solution) (*model.Solution, error) {
	query := `
